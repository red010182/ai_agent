...
│   │   ├── App.tsx
│   │   ├── components/
│   │   │   ├── ChatSidebar.tsx  # 聊天室列表
│   │   │   ├── ChatWindow.tsx   # 對話主區
│   │   │   ├── MessageBubble.tsx
│   │   │   ├── ThinkingBlock.tsx # 可折疊思考過程
│   │   │   ├── SqlConfirmCard.tsx
│   │   │   └── SqlRecord.tsx
│   │   ├── hooks/
...
[候選 case 的 symptom]
case_2: tool xxx issue + foup 沒派滿 + 系統A異常
case_3: tool xxx issue + foup 沒派滿 + 系統A正常 + mask 不在機台內
case_4: tool xxx issue + foup 沒派滿 + 系統A正常 + mask 在機台內
...
POST /api/sessions/{session_id}/chat
  → 發送訊息，SSE 串流回覆
  Request body: { "message": "tool xxx issue" }
  Response: text/event-stream
```

### SSE 串流格式

每個事件為 JSON，前端依 `type` 處理：

```jsonc
// 文字片段（逐字串流，顯示在對話氣泡）
data: {"type": "text_delta", "content": "根據 SOP..."}

// SQL 預覽（等待用戶確認，顯示為確認卡片）
...
// ── 透明度事件（更新右側面板，不顯示在對話主區）──

// 路由決策：Vector Search 結果
data: {
  "type": "trace_routing",
  "matched_sop": "productivity_lost.md",
  "matched_case": "case_1",
  "case_title": "Tool xxx issue",
  "score": 0.87,
  "mode": "sop"           // "sop" | "fallback_chat"
}
...
data: {
  ...
  "known_facts": [
    "原始症狀：tool xxx issue",
    "case_1 查詢結果：foup_count = 3，未派滿（規格需 8）"
  ]
}
...
data: {
  "type": "trace_sql",
  "template": "SELECT foup_id FROM foup_status WHERE equipment_id = '{equipment_id}'",
  "filled": "SELECT foup_id FROM foup_status WHERE equipment_id = 'EQ-4721'",
  "result_rows": 3,
  "result_preview": [
    {"foup_id": "FP-001", "status": "normal"},
    {"foup_id": "FP-002", "status": "normal"},
    {"foup_id": "FP-003", "status": "missing"}
  ]
}
...
data: {
  "type": "trace_decision",
  "candidates": [
    {"case_id": "case_2", "symptom": "xxx issue + foup未派滿 + 系統A異常"},
    {"case_id": "case_3", "symptom": "xxx issue + foup未派滿 + 系統A正常 + mask不在機台"},
    {"case_id": "case_4", "symptom": "xxx issue + foup未派滿 + 系統A正常 + mask在機台"}
  ],
  "chosen": "case_4",
  "reason": "已知系統A正常且mask在機台內"
}
```
...
- 所有路由掛載在 `/api` prefix
- `POST /sessions/{session_id}/chat` 呼叫 `agent` 並以 SSE 回傳
- 用戶回覆 `yes` / `no` 也走同一個 chat 端點，由 session state 判斷當前等待確認的動作
- CORS 設定允許前端來源（內網環境，可設定為 `*`）

### `api/sse.py` 規格

負責將 agent 執行過程轉換為 SSE 事件串流，trace 事件的發送時機：

| 時機 | 發送事件 |
|------|----------|
| `router.route()` 完成後 | `trace_routing` |
| 載入新 case 時 | `trace_case` |
| `append_known_fact()` 被呼叫後 | `trace_facts`（傳完整清單）|
| 用戶確認 SQL、執行完成後 | `trace_sql`（含結果）|
| LLM 條件比對完成後 | `trace_decision` |
| LLM 回覆文字逐步生成 | `text_delta` |
| action 含 SQL 等待確認 | `sql_confirm` |
| 需要補充參數 | `ask_user` |
| 流程結束 | `done` |
...
- **狀態管理**：Zustand（輕量，無需 Redux）
- **Markdown 渲染**：`react-markdown` + `react-syntax-highlighter`（SQL 高亮）
- **建置工具**：Vite，`npm create vite@latest frontend -- --template react-ts`
- **樣式**：Tailwind CSS v4（`@tailwindcss/vite` plugin），與 Ant Design 並用；Tailwind 負責佈局與間距，Ant Design 負責互動元件
- **開發時 proxy**：`vite.config.ts` 設定 `/api` proxy 到 FastAPI，避免 CORS
...
2. **Agent 回覆**：左對齊氣泡，支援 Markdown 渲染

3. **Agent 思考過程（可折疊區塊）**
   - 緊接在每則 Agent 回覆下方
   - 預設收起，標題列顯示「🤖 Agent 思考過程」+ 展開箭頭
   - 展開後顯示 4 個子區塊（每個也可獨立展開/收起）：

   **① 路由決策**
   - 匹配的 SOP 檔名、case_id、case_title
   - Vector Search score（視覺化進度條，0~1）
   - fallback 模式時顯示「未找到對應 SOP（score: 0.42）」

   **② 執行軌跡**
   - case 推進的時間軸清單
   - 格式：`✓ case_1 Tool xxx issue` → `→ case_4 Mask 在機台內`
   - 當前 case 高亮，已完成打勾

   **③ 已知狀態（known_facts）**
   - 條列式清單
   - 每次更新時若區塊已展開，新增項目有短暫黃色 highlight 動畫
   ...
   **④ SQL 記錄**
   ...
   - 子卡片顯示三段（可獨立折疊）：
     - 原始 template（語法高亮）
     - 填入參數後的 SQL（語法高亮）
     - 查詢結果（HTML table，超過 10 筆顯示前 10 筆 + 「共 N 筆」）

4. **SQL 確認卡片**：含語法高亮 SQL + 「確認執行」/「取消」按鈕

5. **Ask user 卡片**：醒目提示框，說明需要哪個參數

**SSE 事件處理：**
- `text_delta`：append 到當前 agent 氣泡
- `sql_confirm`、`ask_user`：在對話區插入對應卡片
- `trace_routing`、`trace_case`、`trace_facts`、`trace_sql`、`trace_decision`：
  寫入當前 agent 回覆對應的「思考過程」區塊資料，若區塊已展開則即時更新 DOM
- `done`：關閉 EventSource，啟用輸入框，最終化思考過程區塊

**其他：**
- **開發環境**：`npm run dev` 啟動 Vite dev server（port 5173），透過 proxy 打到 FastAPI
- **生產環境**：`npm run build` 產出 `frontend/dist/`，由 FastAPI `StaticFiles` 掛載 serve
- 每個 agent 回覆有獨立的思考過程區塊，歷史訊息的思考過程永久保留可查
- 每個聊天室的對話歷史（含思考過程資料）存在前端 JS 的 `chatStore`（記憶體）
...
9. `api/routes.py` + `api/sse.py` — FastAPI 端點與 SSE 串流
10. `frontend/` — `npm create vite` 建腳手架，安裝 antd + zustand + react-markdown，實作元件
11. `main.py` — FastAPI app 入口，掛載 API 與靜態檔案，完整端對端測試
...
- [ ] 多個聊天室並行，session 狀態互不干擾
- [ ] SSE 串流正確逐字回覆，`done` 事件後連線正常關閉
- [ ] SQL confirm 在前端顯示為確認卡片，點擊按鈕自動送出 yes / no
- [ ] 前端可新增、切換、刪除聊天室
- [ ] 每則 Agent 回覆下方有「思考過程」可折疊區塊
- [ ] 路由決策顯示 SOP、case、score 進度條
- [ ] 執行軌跡顯示 case 推進時間軸，當前 case 高亮
- [ ] known_facts 即時更新，新增項目有 highlight 動畫
- [ ] SQL 記錄展示 template → 填入參數 → 查詢結果三段完整過程
- [ ] 歷史訊息的思考過程永久保留可查