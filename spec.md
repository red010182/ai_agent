...
POST /api/sessions/{session_id}/chat
  → 發送訊息，SSE 串流回覆
  Request body: { "message": "tool scanner lost" }
...
data: {
  "type": "trace_decision",
  "candidates": [
    {"case_id": "case_2", "symptom": "scanner lost + foup未派滿 + 系統A異常"},
    {"case_id": "case_3", "symptom": "scanner lost + foup未派滿 + 系統A正常 + mask不在機台"},
    {"case_id": "case_4", "symptom": "scanner lost + foup未派滿 + 系統A正常 + mask在機台"}
  ],
  "chosen": "case_4",
...
3. **Agent 思考過程（可折疊區塊）**
   ...
   **② 執行軌跡**
   - case 推進的時間軸清單
   - 格式：`✓ case_1 Tool Scanner Lost` → `→ case_4 Mask 在機台內`
   - 當前 case 高亮，已完成打勾
